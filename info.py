...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002492413086').split()] #Channel id for auto indexing ( make sure bot is admin )
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1002429092558') #Channel / Group Id for force sub ( make sure bot is admin )
...
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002409302148') # request channel id ( make sure bot is admin )
...
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://usdpay:usdpay@cluster0.6dumo.mongodb.net/?retryWrites=true&w=majority")
...
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/akimax_03')
TUTORIAL = environ.get('TUTORIAL', 'https://t.me/akimax_03') # Tutorial video link for opening shortlink website 
IS_TUTORIAL = bool(environ.get('IS_TUTORIAL', True))
MSG_ALRT = environ.get('MSG_ALRT', 'ᴍᴀɪɴᴛᴀɪɴᴇᴅ ʙʏ : 𝚅𝚎𝚐𝚊𝚖𝚘𝚟𝚒𝚎𝚜𝚗𝚎𝚠𝚒𝚗')
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1002409302148')).split()]
